...
import static org.tachacoin.wallet.TachacoinApplication.REALM_NAME;
...
public class WearListCallListenerService extends WearableListenerService implements GoogleApiClient.ConnectionCallbacks, GoogleApiClient.OnConnectionFailedListener, MessageApi.MessageListener {
    ...
    public void onMessageReceived(MessageEvent messageEvent) {
        ...
        if(messageEvent.getPath().contains("/get_history")) {
            ...
            currentAddress = TachacoinSharedPreference.getInstance().getCurrentAddress(getApplicationContext());
            sendData();
        } else {
            stopSelf();
        }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    final String balance = TachacoinSharedPreference.getInstance().getBalanceString(WearListCallListenerService.this);
                    final String uncBalance = TachacoinSharedPreference.getInstance().getUnconfirmedBalanceString(WearListCallListenerService.this);
                    final String address = TachacoinSharedPreference.getInstance().getCurrentAddress(WearListCallListenerService.this);
                    Gson gson = new Gson();
