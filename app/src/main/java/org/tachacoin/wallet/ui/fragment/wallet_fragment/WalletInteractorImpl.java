...
public class WalletInteractorImpl implements WalletInteractor {
    ...
    public Observable<HistoryResponse> getHistoryList(int limit, int offest) {
        return TachacoinService.newInstance().getHistoryListForSeveralAddresses(getAddresses(), limit, offest);
    }
    ...
    public String getAddress() {
        ...
        if (!TextUtils.isEmpty(s)) {
            TachacoinSharedPreference.getInstance().saveCurrentAddress(context, s);
        }
        return s;
    }
    ...
    public Observable<List<TransactionReceipt>> getTransactionReceipt(String txHash) {
        return TachacoinService.newInstance().getTransactionReceipt(txHash);
    }
