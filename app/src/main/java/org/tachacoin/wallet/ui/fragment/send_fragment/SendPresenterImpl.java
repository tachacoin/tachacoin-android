...
public class SendPresenterImpl extends BaseFragmentPresenterImpl implements SendPresenter {
    ...
    public void send() {
        if (mNetworkConnectedFlag) {
            ...
            if (feeDouble < minFee || feeDouble > maxFee) {
                ...
                getView().setAlertDialog(org.tachacoin.wallet.R.string.error, R.string.invalid_fee, "Ok", BaseFragment.PopUpType.error);
                return;
            }
            ...
            getView().setAlertDialog(org.tachacoin.wallet.R.string.no_internet_connection, org.tachacoin.wallet.R.string.please_check_your_network_settings, "Ok", BaseFragment.PopUpType.error);
        }
    }
    ...
    public void onPinSuccess(String passphrase) {
        ...
        if (currency.getName().equals("Tachacoin " + getView().getStringValue(org.tachacoin.wallet.R.string.default_currency))) {
        ...
        getInteractor().createAbiMethodParamsObservable(address, resultAmount, "transfer")
                ...
                .subscribe(new Observer<CallSmartContractResponse>() {
                    ...
                    public void onError(Throwable e) {
                        e.printStackTrace();
                        getView().dismissProgressDialog();
                        getView().setAlertDialog(org.tachacoin.wallet.R.string.error, e.getMessage(), "Ok", BaseFragment.PopUpType.error);
                    }

                    @Override
                    public void onNext(CallSmartContractResponse response) {
                        if (!response.getItems().get(0).getExcepted().equals("None")) {
                            getView().setAlertDialog(org.tachacoin.wallet.R.string.error,
    ...
    private void createTx(final String abiParams, final String contractAddress, String senderAddress, final int gasLimit, final int gasPrice, final String fee, final String passphrase) {
        getInteractor().getUnspentOutputs(senderAddress, new SendInteractorImpl.GetUnspentListCallBack() {
            ...
            public void onError(String error) {
                getView().dismissProgressDialog();
                getView().setAlertDialog(org.tachacoin.wallet.R.string.error, error, "Ok", BaseFragment.PopUpType.error);
            }
        });
    }
