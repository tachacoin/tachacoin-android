...
import org.tachacoin.wallet.ui.activity.splash_activity.SplashActivity;
import org.tachacoin.wallet.ui.base.base_activity.BaseActivity;
...
import org.tachacoin.wallet.ui.fragment.wallet_fragment.WalletFragment;
import org.tachacoin.wallet.ui.fragment.wallet_main_fragment.WalletMainFragment;
import org.tachacoin.wallet.utils.CustomContextWrapper;
...
import static org.tachacoin.wallet.ui.base.base_fragment.BaseFragment.BACK_STACK_ROOT_TAG;
...
public class MainActivity extends BaseActivity implements MainActivityView {
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (intent != null && intent.getAction() != null) {
            switch (intent.getAction()) {
                case TachacoinIntent.OPEN_FROM_NOTIFICATION:
                    openRootWallet();
                    break;
                default:
                    break;
    ...
    public boolean checkTouchIdEnable() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if (TachacoinSharedPreference.getInstance().isTouchIdEnable(getContext())) {
    ...
    public void initializeViews() {
        ...
        switch (intent.getAction()) {
            case TachacoinIntent.SEND_FROM_SDK:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = intent.getStringExtra(TachacoinIntent.SEND_ADDRESS);
                mAmountForSendAction = intent.getStringExtra(TachacoinIntent.SEND_AMOUNT);
                mTokenAddressForSendAction = intent.getStringExtra(TachacoinIntent.SEND_TOKEN);
                break;
            case NfcAdapter.ACTION_NDEF_DISCOVERED:
                getPresenter().setSendFromIntent(true);
                mAddressForSendAction = "TKvB7xXGniHnt2r91aaw1tHEwLYTg25e4T";
    ...
    protected void attachBaseContext(Context newBase) {
        super.attachBaseContext(CustomContextWrapper.wrap(newBase, TachacoinSettingSharedPreference.getInstance().getLanguage(newBase)));
    }

    public TachacoinApplication getTachacoinApplication() {
        return (TachacoinApplication) getApplication();
    }
    ...
    public Realm getRealm(){
        return ((TachacoinApplication)getApplication()).getRealm();
    }
}
