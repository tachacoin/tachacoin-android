...
public class SendInteractorImpl implements SendInteractor {
    ...
    public void getUnspentOutputs(final GetUnspentListCallBack callBack) {
        TachacoinService.newInstance().getUnspentOutputsForSeveralAddresses(KeyStorage.getInstance().getAddresses())
    ...
    public void getUnspentOutputs(String address, final GetUnspentListCallBack callBack) {
        ...
        TachacoinService.newInstance().getUnspentOutputs(address)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new Subscriber<List<UnspentOutput>>() {
                    @Override
        ...
        getUnspentOutputs(from, new GetUnspentListCallBack() {
            ...
            public void onSuccess(List<UnspentOutput> unspentOutputs) {
                ...
                try {
                    ...
                    callBack.onError(mContext.getString(org.tachacoin.wallet.R.string.invalid_tachacoin_address));
                    return;
                }
                ...
                if (overFlow.doubleValue() < amount.doubleValue()) {
                    callBack.onError(mContext.getString(org.tachacoin.wallet.R.string.you_have_insufficient_funds_for_this_transaction));
                    return;
                }
                ...
                if (minimumFee.doubleValue() > fee.doubleValue()) {
                    callBack.onError(mContext.getString(R.string.insufficient_fee_lease_use_minimum_of) + " " + minimumFee.toString() + " TACHACOIN");
                    return;
                }
    ...
    public void sendTx(String txHex, final SendTxCallBack callBack) {
        TachacoinService.newInstance().sendRawTransaction(new SendRawTransactionRequest(txHex, 1))
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
    ...
    public Observable<CallSmartContractResponse> callSmartContractObservable(Token token, String hash, String fromAddress) {
        return TachacoinService.newInstance().callSmartContract(token.getContractAddress(), new CallSmartContractRequest(new String[]{hash}, fromAddress));
    }
